...
public class PartitionKSubsetsII {
    ...
    public static boolean dfs(int[] nums, int begin, int target, int curSum, int k, boolean[] used) {
        ...
        for (int i = begin; i >= 0; i--) {
            ...
            used[i] = true;
            if (curSum + nums[i] < target && dfs(nums, i-1, target, curSum + nums[i], k, used)) {
                return true;
            }
            if (curSum + nums[i] == target && dfs(nums,nums.length - 1, target,0,k - 1, used)) {
                return true;
            }
            used[i] = false;
    ...
    public static void main(String[] args) {
        PartitionKSubsetsII obj = new PartitionKSubsetsII();
